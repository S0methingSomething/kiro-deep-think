...
info() { echo "${BLUE}$*${NC}"; }
                                                              # Header
info "========================================="              info "   Kiro Deep Thinker Agent Installer"
info "========================================="              echo ""

# Check requirements
info "🔍 Checking requirements..."                                                                                          if ! command -v kiro-cli >/dev/null 2>&1; then
...
if ! command -v jq >/dev/null 2>&1; then
  warn "jq not found. Task manager will have limited functionality."                                                          echo "   Install: sudo apt install jq  (or brew install jq on macOS)"                                                     fi
                                                              if ! command -v python3 >/dev/null 2>&1; then
  warn "python3 not found. Advanced search features will be unavailable."
fi                                                            
# Check authentication                                        info ""
info "🔐 Checking Kiro authentication..."
if ! kiro-cli settings chat.defaultModel >/dev/null 2>&1; then  err "Not authenticated with Kiro CLI. Please run:"
...
mkdir -p "$AGENT_DIR"
...
else                                                            # Remote installation (curl | bash)
  ...
  download() {
    ...
    if command -v curl >/dev/null 2>&1; then
      curl -fsSL "$url" -o "$dest"                                elif command -v wget >/dev/null 2>&1; then
      wget -q "$url" -O "$dest"
    else                                                            err "Neither curl nor wget found. Cannot download files."
      exit 1
    fi
  }                                                           
  download "deep-thinker.json"                                  download "deep-thinker-prompt.md"
...
info ""                                                       info "✅ Installation complete!"                              echo ""                                                       echo "Agent installed to: ${AGENT_DIR}"
echo ""
echo "Usage:"
...
echo "  • Interweaved research and code iteration"            echo "  • Production-ready task manager with full history"
